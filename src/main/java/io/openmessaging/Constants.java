package io.openmessaging;

/**
 * Created by huzhebin on 2019/07/23.
 */
public class Constants {
    // writer缓冲区
    public final static int WRITER_CAP = 16 * 1024;
    // data写入缓冲区
    public final static int DATA_CAP = 32 * 1024 * 1024;
    // value写入缓冲区
    public final static int VALUE_CAP = 32 * 1024 * 1024;
    // data 的大小
    public final static int DATA_SIZE = 32;

    public final static int DATA_NUM = 1024;

    public final static int DATA_BUF_NUM = 40;
    // value 的大小
    public final static int VALUE_SIZE = 8;

    public final static int VALUE_NUM = 1024;

    public final static int VALUE_BUF_NUM = 80;

    //public final static String URL = "/users/huzhebin/data/";

    public final static String URL = "/alidata1/race2019/data/";
}
